function aaa = ARBITRAGE_NPC_COMPARISON_2_MEGA(bat_cap, pbidi_cap_ch,pbidi_cap_disch)
%%   Arbitrage Comparison Script - Day (24) Ahead - Meeting Building loads & Exporting Excess Energy

load RTP_analysis.mat % To be used by the function
...
minHourPowerLimit_margin = 1; % allow 1 kW as a margin error
maxHourPowerLimit_margin = 1; % allow 1 kW as a margin error
...
cost_converter_excVAT = 170 * abs(pbidi_cap_disch); 
cost_converter_final = cost_converter_excVAT * (1+ VAT);
...
if exclusions == true % then specific hours will be excluded from charging or discharging based on the respective building loads       
   % this will ensure that hours will be removed from the DISCHARGING phase if during a weekday, the building load is lower than the set limit. Weekends are NOT excluded from discharging.
        ...
        % The following line will ensure that there are is no discharging during weekends and bank holidays.
...
if(isempty(RTP_MINRANGE_TO_MAXRANGE(HourIndexExamined_MINRANGE_TO_MAXRANGE == 1)) == 1)
    ...
    % Provision for negative electricity prices.
     ...
     if (MCproduction + MCmargin) < (RTP_retail(maxHourIndex)) && (minHourIndex ~= maxHourIndex) 
        ...
        if totalenergy(maxHourIndex)/(pbidi_eff * nbattd) >= -pbidi_cap_disch
        bottleneck(1) = energy_exchange(maxHourIndex)- pbidi_cap_disch ;
        else
        bottleneck(1) = energy_exchange(maxHourIndex) + totalenergy(maxHourIndex)/(pbidi_eff * nbattd) ; 
        end
        ...
        if totalenergy(maxHourIndex) >= -pbidi_cap_disch*nbattd*pbidi_eff
            
           if (energy_exchange(maxHourIndex) <= pbidi_cap_disch)
              HourIndexExamined(maxHourIndex) = 0;
           end
           else
           if energy_exchange(maxHourIndex) <= -totalenergy(maxHourIndex)/(nbattd*pbidi_eff)
              HourIndexExamined(maxHourIndex) = 0;
           end
        end
...
%%
figure14156244496 = figure('visible','off');
...
fprintf('For battery storage system %.2f kWh (%.2f/%.2f kW)\n',bat_cap,pbidi_cap_ch/(nbattch*pbidi_eff_RECTIFIER),pbidi_cap_disch);
aaa = [totalenergy_WITH_annual,totalcost_WITH_annual,exports_annual,revenues_WITH_annual,net_cost_WITH_annual,sum(battery_discharge_REAL),power_shifted_annual_scalar,battery_lifetime,(power_shifted_annual_scalar/sum(totalenergy))*100,(power_shifted_annual_scalar/peak_loads_scalar)*100, 0.02, net_cost_WITH_alternative_annual/2500, NPC_without_storage, NPC_with_storage, NPC_with_storage_alternative, LCOE_without_storage, LCOE_with_storage, LCOE_with_storage_alternative, financial_motive_shifted, financial_motive_exports, financial_motive_shifted_exports, financial_motive_shifted_alt, financial_motive_exports_alt, financial_motive_shifted_exports_alt]';
%%  Save all the results (Data_Analysis/RTP/Arbitrage)
fname3 = sprintf('ARBITRAGE_NPC_COMPARISON_2_%.0f_%.0f_%.0f_%s.mat',bat_cap,pbidi_cap_ch/(nbattch*pbidi_eff_RECTIFIER),-pbidi_cap_disch,ScenarioID);
save(fname3)
